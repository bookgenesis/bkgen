...
class EPUB(ZIP, Source):
    ...
    def images(self):
        ...
        for item in opf.xpath(
                opf.root, "opf:manifest/opf:item[contains(@media-type,'image')]", namespaces=NS):
            zf_path = os.path.relpath(
                os.path.join(os.path.dirname(opf.fn), item.get('href')), self.fn).replace(
                    '\\', '/')
    ...
    def resources(self, path=None, opf=None):
        ...
        if found_cover == False:
            ...
            if cover_id is not None:
                item = opf.find(
                    opf.root, "opf:manifest/opf:item[@id='%s']" % cover_id, namespaces=NS)
                if item is not None:
    ...
    def opf_manifest_item(C, opf_path, href, mediatype=None):
        item = C.OPF.item({
            ...
            href,
            'media-type':
            mediatype or mimetypes.guess_type(os.path.join(opf_path, href))[0]
            or C.MEDIATYPES.get(os.path.splitext(href)[1]) or '',
        })
        item.tail = '\n\t'
        return item
    ...
    @classmethod
    def nav_toc_from_spine_items(C,
                                 output_path,
                                 spine_items,
        ...
        for spine_item in spine_items:
            ...
            item_title = (spine_item.get('title')
    ...
    def opf_package_metadata(C, metadata, xml_lang='en-US', cover_src=None):
        ...
        for date_elem in metadata_elem.xpath("dc:date", namespaces=C.NS):
            if (date_elem.text or '').strip() == '':
                ...
                popped = Dict()
                for k in [k for k in date_elem.attrib.keys() if k not in ALLOWED_DATE_PROPERTIES]:
    ...
    def opf_spine(C, output_path, spine_items=None, manifest=None, ncx_href=None):
        ...
        if spine_items is not None:
            ...
            spine = C.OPF.spine(
                *[C.OPF.itemref(idref=item.get('id')) for item in manifest.getchildren()])
        else:
            raise ValueError(
    ...
    def opf_spine_itemref(C, spine_item):
        ...
        itemref = C.OPF.itemref({
            'idref':
            spine_item.get('idref')  # use @idref if given
    ...
    def make_mimetype_file(C, output_path, mimetype=None):
        ...
        t = Text(fn=os.path.join(output_path, 'mimetype'), text=mimetype, encoding='ascii')
        t.write()
        return t.fn
        ...
        x = XML(
            ...
            root=Container.container(
                ...
                Container.rootfiles(*[
                    Container.rootfile({
                        'full-path':
                        os.path.normpath(os.path.relpath(opf_fn, output_path)).replace('\\', '/'),
                        'media-type':
                        C.MEDIATYPES.get('.opf'),
    ...
    def zip_epub(
        ...
        epub.zipfile.write(container_fn,
